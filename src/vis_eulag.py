...
def plot_tzofxy(ds, SETTINGS):
    ...
    for var in varvec:
        ...
        if var=='TH':
            ...
            clev, clev_l = get_colormap_bins_and_labels(max_level=float(SETTINGS['CLEV_TH']))
            ...
            clev, clev_l = get_colormap_bins_and_labels(max_level=float(SETTINGS['CLEV_MFX']))
            norm = BoundaryNorm(boundaries=clev , ncolors=cmap.N, clip=True)
            i=0
            for z in zvec:
                j=0
            ...
            clev, clev_l = get_colormap_bins_and_labels(max_level=float(SETTINGS['CLEV_EP']))
            norm = BoundaryNorm(boundaries=clev , ncolors=cmap.N, clip=True)
            i=0
            for z in zvec:
                j=0
...
def plot_primes_xz(ds, SETTINGS, t, y=0, theta=0):
    ...
    ## U ##
    clev, clev_l = get_colormap_bins_and_labels(max_level=float(SETTINGS['CLEV_U']))
    norm = BoundaryNorm(boundaries=clev , ncolors=cmap.N, clip=True)    
    ...
    ## W or V ##
    clev, clev_l = get_colormap_bins_and_labels(max_level=float(SETTINGS['CLEV_W']))
    norm = BoundaryNorm(boundaries=clev , ncolors=cmap.N, clip=True) 
    ...
    if SETTINGS['CLEV_TH'] == 'CLEV_01' or SETTINGS['CLEV_TH'] == 'CLEV_05':
      ...
      # PRESSURE ##  
    clev, clev_l = get_colormap_bins_and_labels(max_level=float(SETTINGS['CLEV_P']))
    norm = BoundaryNorm(boundaries=clev , ncolors=cmap.N, clip=True)
...
def plot_primes_xy(ds, SETTINGS, t, z=0, theta=0):
    ...
    clev, clev_l = get_colormap_bins_and_labels(max_level=float(SETTINGS['CLEV_W']))
    norm = BoundaryNorm(boundaries=clev , ncolors=cmap.N, clip=True) 
    pcMesh1 = ax1.pcolormesh(ds.xcr, ds.ycr, ds.v[t,z,:,:],
                            cmap=cmap, norm=norm, shading='nearest')
    cbar = fig.colorbar(pcMesh1, ax=ax1, orientation='horizontal', ticks=clev_l, pad=0.07)
    # cbar.set_label("$w'$ (vertical velocity) / m s$^{-1}$")
    cbar.set_label("$v'$ (meridional velocity) / m s$^{-1}$")
    ...
    # pcMesh3 = ax3.pcolormesh(ds.xcr[y], ds.zcr[t,:,y,:], ds.pprime[t,:,y,:],
    #                         cmap=cmap, norm=norm, shading='nearest')
    pcMesh3 = ax3.pcolormesh(ds.xcr, ds.ycr, ds.pprime[t,z,:,:],
                            cmap=cmap, norm=norm, shading='nearest')
    # pcMesh3 = ax3.pcolormesh(ds.xcr[y], ds.zcr[t,:,y,:], ds.vprime[t,:,y,:],
    #                         cmap=cmap, norm=norm, shading='nearest')
    cbar = fig.colorbar(pcMesh3, ax=ax3, orientation='horizontal', ticks=clev_l, pad=0.07)
...
def plot_fluxes_xz(ds, SETTINGS, t, y=0):
    ...
    ## EFx ##
    clev, clev_l = get_colormap_bins_and_labels(max_level=float(SETTINGS['CLEV_EFX']))
    norm = BoundaryNorm(boundaries=clev , ncolors=cmap.N, clip=True)
    ...
    ## EFz (-MF*U)##
    clev, clev_l = get_colormap_bins_and_labels(max_level=float(SETTINGS['CLEV_EFZ']))
    norm = BoundaryNorm(boundaries=clev , ncolors=cmap.N, clip=True)
...
def plot_vertical_profiles(ds, SETTINGS, y=0):
    ...
    l_prof = np.sqrt(N_prof**2 / ds['ue'][t,:,y,x]**2 - 1/ds['ue'][t,:,y,x] * dudz2)
    l_prof_simp = np.sqrt(N_prof**2 / ds['ue'][t,:,y,x]**2)
    ax1.plot(l_prof, ds.zcr[t,:,y,x], lw=2, color='goldenrod', label='$l$ t0')
    ax1.plot(l_prof_simp, ds.zcr[t,:,y,x], lw=2, color='indianred', label='$l$ (no curv) t0')
    ...
    # - reflection of waves - analysis of horizontal wavelength - #
    lambdax_vec = [10,20,50,100,400] # km
    lambdax_vec = np.asarray(lambdax_vec)
    k_vec       = 2*np.pi / (lambdax_vec * 1000)
    ...
    for t in range(0,np.shape(ds['u'])[0],2):
        ...
        l_prof = np.sqrt(N_prof**2 / ds['u'][t,:,y,x]**2 - 1/ds['u'][t,:,y,x] * dudz2)

        if (t==np.shape(ds['u'])[0]-1):
            ax0.plot(u_avg, ds.zcr[t,:,y,x], lw=2, color='black', alpha=0.6, label='$\hat{u}$')
            ax1.plot(l_prof, ds.zcr[t,:,y,x], lw=2, color='black', alpha=0.6, label='$l$ t6')
            ...
            # ax1.vlines(x=k_vec[0],ymin=0,ymax=ds.zcr[t,-1,y,x],color='grey',ls='--', label='$\lambda_x$=10km') 
            ax1.vlines(x=k_vec[1],ymin=0,ymax=ds.zcr[t,-1,y,x],color='silver',ls='--', label='$\lambda_x$=20km') 
            ax1.vlines(x=k_vec[2],ymin=0,ymax=ds.zcr[t,-1,y,x],color='darkgrey',ls='--', label='$\lambda_x$=50km') 
            ax1.vlines(x=k_vec[3],ymin=0,ymax=ds.zcr[t,-1,y,x],color='grey',ls='--', label='$\lambda_x$=100km')  
            ax1.vlines(x=k_vec[4],ymin=0,ymax=ds.zcr[t,-1,y,x],color='black',ls='--', label='$\lambda_x$=400km')        
        else:
            ax0.plot(u_avg, ds.zcr[t,:,y,x], color='black',alpha=0.6)
            ax1.plot(l_prof, ds.zcr[t,:,y,x], color='black',alpha=0.6)
...
def plot_eliassenPalm_theorem(ds, SETTINGS):
    "Visualizes Eliassen–Palm relation for last time step and y"
    fig, ax0 = plt.subplots(figsize=FIGSIZE_QQ_PLOT)
    y=int(ds.ny/2)
    t = 5
    zmax = ds.nz-1
    z = np.linspace(0,ds['zcr'][t,zmax,y,0].values,zmax) # ds['dx']=200
    
    # array.stack(z=("x", "y"))
    # --------- Calculate fluxes -------------------- #  
    uw = ds['w'][t,:zmax,y,:] * (ds['u'][t,:zmax,y,:]-ds['ue'][t,:zmax,y,:]) # h9=h3*h6
    vw = ds['w'][t,:zmax,y,:] * (ds['v'][t,:zmax,y,:]-ds['ve'][t,:zmax,y,:])
    # MF_U = mfx * ds['u'][t,:zmax,y,:] + mfy * ds['v'][t,:zmax,y,:] # = -EFz
    efz = ds['w'][t,:zmax,y,:] * ds['pprime'][t,:zmax,y,:] # h13, EFz1
    
    # --------- FILTER -------------------- #
    ## INTERPOLATE TO REGULAR VERTICAL GRID ##
    rho = interp_elev_to_z(ds['rh0'][t,:zmax,y,:],ds.zcr[t,:zmax,y,:],z)
    u = interp_elev_to_z(ds['u'][t,:zmax,y,:],ds.zcr[t,:zmax,y,:],z)
    v = interp_elev_to_z(ds['v'][t,:zmax,y,:],ds.zcr[t,:zmax,y,:],z)
    uw = interp_elev_to_z(uw,ds.zcr[t,:zmax,y,:],z)
    vw = interp_elev_to_z(vw,ds.zcr[t,:zmax,y,:],z)
    # MF_U = interp_elev_to_z(MF_U,ds.zcr[t,:zmax,y,:],z)
    efz = interp_elev_to_z(efz,ds.zcr[t,:zmax,y,:],z)

    ## AVERAGE/FILTER VERTICAL AND HORIZONTAL OVER ONE WAVELENGTH
    # nz_avg = int(2*np.pi*wind_f/ds.bv/ds.dz00)+1
    nz_avg = 40 # *0.25km -> 10km
    
    uw = filter_1Dz(uw,nz_avg,mode=SETTINGS['FILTER_MODE'])
    vw = filter_1Dz(vw,nz_avg,mode=SETTINGS['FILTER_MODE'])
    # MF_U = filter_1Dz(MF_U,nz_avg,mode=SETTINGS['FILTER_MODE'])
    efz = filter_1Dz(efz,nz_avg,mode=SETTINGS['FILTER_MODE'])
    
    MF_U = rho*uw * u + rho*vw * v # = -EFz
    ...
    efz = efz[:,n_sponge:ds.nx-n_sponge] # .mean(axis=1)
    MF_U = MF_U[:,n_sponge:ds.nx-n_sponge] # .mean(axis=1)
    ...
    # Save fig
    fig.tight_layout()
    fig_title = 'EP-relation-L10km' + '.png'
    fig.savefig(SETTINGS['FILE_LOCATION'] + '/' + SETTINGS['PLOT_FOLDER'] + '/' + fig_title, facecolor='w', edgecolor='w',
                format='png', dpi=150) # orientation='portrait
...
def plot_surf(ds, SETTINGS, y=0):
    ...
    ## PLOT LINEAR THEORY ##
    drag_linear_theory(ds,SETTINGS)
...
def drag_linear_theory(ds,SETTINGS):
    ...
    for t in tdrag:
        ...
        # - Plot - #
        tstr = str(int(t*ds.nplot*ds.dt00/3600)) + 'h' # h
        if (setlabel==0):
            ax0.scatter(x_regime, drag, marker='D', s=50, color='indianred', label="p'deta/dx")
            ax0.scatter(x_regime, drag_angMom, marker='x', s=50, color='y', label="u'w'+fv'eta'")
        else:
            ax0.scatter(x_regime, drag, marker='D', s=50, color='indianred')
            ax0.scatter(x_regime, drag_angMom, marker='x', s=50, color='y')
    ...
    dragdy_angMom = []
    t=6
    # set y range manually for simplicity

    if ds.irely:
        ny_sponge=int(ds.dyab/ds.dy00)
    else:
        ny_sponge=0

    for y in range(ny_sponge,ds.ny-ny_sponge,1):
        drag = integrate.trapezoid(ds.pprime[t,z,y,n_sponge:ds.nx-n_sponge] * ds.dzdx_surf[y,n_sponge:ds.nx-n_sponge], 1000*ds.xcr[y,n_sponge:ds.nx-n_sponge]) # 0.3576
        ...
        dragdy.append(drag)
        dragdy_angMom.append(drag_angMom)


    dragdy = np.asarray(dragdy).mean()
    dragdy_angMom = np.asarray(dragdy_angMom).mean()

    # - Plot - #
    tstr = str(int(t*ds.nplot*ds.dt00/3600)) + 'h' # h
    ax0.scatter(x_regime, dragdy, marker='D', s=50, color='black')
    ax0.scatter(x_regime, dragdy_angMom, marker='x', s=50, color='black')
    ax0.annotate(tstr, (x_regime+seperation, dragdy), color='black',transform=ax0.transAxes)
    ax0.annotate(tstr, (x_regime+seperation, dragdy_angMom), color='black', transform=ax0.transAxes) 
    """
